...
//	cvUndistortPoints(proj_dist_points, proj_undist_points, sl_calib->proj_intrinsic, sl_calib->proj_distortion, NULL, NULL);
	cvUndistortPoints(proj_dist_points, proj_undist_points, sl_calib->proj_intrinsic, NULL, NULL, NULL);
   ...
   sl_calib->proj_planes[0]=cvCreateMat(sl_params->proj_h, 4, CV_32FC1);
   sl_calib->proj_planes[4]=cvCreateMat(sl_params->proj_w, 4, CV_32FC1);
   for(int c=0; c<sl_params->proj_w; c++){
   ...
   for(int i=0; i<4; i++)
      sl_calib->proj_planes[4]->data.fl[4*c+i] = plane[i];
   cvReleaseMat(&points);
   }
   ...
   for(int r=0; r<sl_params->proj_h; r++){
      ...
      cvFitPlane(points, plane);
      for(int i=0; i<4; i++)
         sl_calib->proj_planes[0]->data.fl[4*r+i] = plane[i];
      cvReleaseMat(&points);
   }
   ...
   for(int j=0;j<8;j++){
      ...
      case 1      :   {col=0;row=1;distant=2;direct=1;}   break;
      case 2      :   {col=1;row=0;distant=1;direct=1;}   break;
      case 3      :   {col=1;row=0;distant=2;direct=1;}   break;
      case 4      :   {col=0;row=1;distant=0;direct=1;}   break;
      case 5      :   {col=1;row=0;distant=2;direct=-1;}   break;
      case 6      :   {col=1;row=0;distant=1;direct=-1;}   break;
      case 7      :   {col=0;row=1;distant=2;direct=-1;}   break;
      default      :   break;
      }
      if((j!=0)&&(j!=4))
         SetDiagonalPlanes(sl_params,sl_calib,row,col,distant,direct,j);
   }
...
int n_cout(int row,int col,int r_distant,int c_distant, int width, int height){
	int num = 0;
	while(1){
		if((0<=col)&&(col<=width-1)&&(row<=height-1)&&(0<=row)){
			row+=r_distant; col+=c_distant; num++;
		}
		else
			break;
	}
...
int SetPlanes(int row,int col,int h_grow,int w_grow,float scale,int count,int p_num,struct slParams* sl_params,struct slCalib* sl_calib){

	int num = n_cout(row,col,h_grow,w_grow,sl_params->proj_w,sl_params->proj_h);
	if(num>2){	
		CvMat* points = cvCreateMat(num+1, 3, CV_32FC1);
		int proj_nelems = sl_params->proj_w*sl_params->proj_h;
		for(int k=0;k<num;k++){
			int ri = (sl_params->proj_w)*(row+h_grow*k)+(col+w_grow*k);
				for(int i=0; i<3; i++)
					points->data.fl[3*k+i] = 
						sl_calib->proj_center->data.fl[i] + scale*sl_calib->proj_rays->data.fl[ri+proj_nelems*i];
		}
			for(int i=0; i<3; i++)
				points->data.fl[3*num+i] = sl_calib->proj_center->data.fl[i];
			float plane[4];
			cvFitPlane(points, plane);
			for(int i=0; i<4; i++)
				sl_calib->proj_planes[p_num]->data.fl[4*count+i] = plane[i];
			cvReleaseMat(&points);
		}
	return 0;
	}
...
int SetDiagonalPlanes(struct slParams* sl_params, struct slCalib* sl_calib,int row,int col,int distant,int direct,int p_num){

	float scale = 0;
	for(int i=0; i<3; i++)
		scale += pow((float)sl_calib->proj_center->data.fl[i],(float)2.0);
	scale = sqrt(scale);
	int width = sl_params->proj_w; 
	int height = sl_params->proj_h; 
	int n_planes = (width*col+height*row)*distant + (height*col+width*row-distant);
	sl_calib->proj_planes[p_num]=cvCreateMat(n_planes, 4, CV_32FC1);
	for(int c=0; c<n_planes; c++){
		if(0<direct){
				if(c<width*col+height*row)
					for(int k =0;k<distant;k++){
						int ro =(c*row+k*col); int co = (c*col+k*row); 
						int count = (c*distant+k);
						SetPlanes(ro,co,(distant*col-row),(-col+distant*row),scale,count,p_num,sl_params,sl_calib);
				}
				else{
					int ro =((height-1)*row+(distant+c-width)*col); int co = ((width-1)*col+(distant+c-height)*row); 
					int count = (c+(width*col+height*row)*(distant-1));
					SetPlanes(ro,co,(distant*col-row),(-col+distant*row),scale,count,p_num,sl_params,sl_calib);
				}
			...
			if(c<col*(height-distant)+height*row)
					...
					int ro =((height-c-1)*col+c*row); int co = (width-1-k)*row; 
					int count = ((c*distant+k)*row+c*col);
					SetPlanes(ro,co,(distant*col-row),(col-distant*row),scale,count,p_num,sl_params,sl_calib);
			}
			else
				for(int k=0;k<(distant*col+row);k++){
					int ro =((distant-1-k)*col+(height-1)*row); int co = ((c-height+distant)*(col-row)+(width-1)*row);
					int count = ((c-height+distant)*(distant-1)+c+k)*col+(c+height*(distant-1))*row;
					SetPlanes(ro,co,(distant*col-row),(col-distant*row),scale,count,p_num,sl_params,sl_calib);
			}
