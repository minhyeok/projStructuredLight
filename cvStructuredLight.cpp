...
int main(int argc, char* argv[])
{
	...
	cvNamedWindow("projWindow", CV_WINDOW_NORMAL);
	...
	cvSetWindowProperty("projWindow", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
	cvWaitKey(1);
...
//			writeConfiguration(configFile, &sl_params);
			...
			printf("\n> Running scanner (view %d) And input Angle...\n", ++scan_index);
			int Angle=0;
			scanf("%d", &Angle);
			runStructuredLight(capture, &sl_params, &sl_calib, scan_index,Angle);
			cvKey = NULL;
		}
		else if(cvKey == 'f'){
			printf("\n> Running all-pattern scanner (view %d)...\n", ++scan_index);
			All_runStructuredLight(capture, &sl_params, &sl_calib, scan_index);
			cvKey = NULL;
		}
		else if(cvKey == 'b'){
			printf("\n> Scanning background...\n");
			...
			cvSet(sl_calib.background_mask, cvScalar(255));
			int Angle=0;
			runBackgroundCapture(capture, &sl_params, &sl_calib,Angle);
			cvKey = NULL;
		}
		else if(cvKey == 'r'){
	...
	for(int j=0;j<8;j++)
		cvReleaseMat(&sl_calib.proj_planes[j]);
